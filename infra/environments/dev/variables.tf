...
variable "cognito_callback_urls" {
  description = <<-EOT
    OAuth callback URLs for the Cognito app client.
    Must include the CloudFront distribution URL (available after the first
    terraform apply) and any local development URLs.
    See docs/deployment-flow.md for the two-step bootstrap process.
  EOT
  type        = list(string)
